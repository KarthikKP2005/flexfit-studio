 ...
 * src/app/admin/companies/[id]/page.tsx — no JSX, styling, trpc call, or
 * logic changed, only the file location. `useParams()` still works
 * correctly here: Next.js resolves route params via React context from
 * the nearest dynamic segment, not from which file defines the
 * component, so this remains scoped to `/admin/companies/[id]` exactly
 * as before. The page itself is now route-level composition only
 * (plan.md item #54's own pattern), wrapping this in `RequireRole`.
 *
